    ...
    node('k2-tools') {
        customContainer('docker') {
            ...
            stage('Run k2 dryrun test through k2-tools image') {
                kubesh "PWD=`pwd` && docker run --name k2tools --rm -i -v ${env.WORKSPACE}/k2-tools -e JOB_BASE_NAME=${env.JOB_BASE_NAME} -e BUILD_ID=${env.BUILD_ID} quay.io/samsung_cnct/k2-tools:latest '/bin/bash' 'k2-tools/k2dryrun.sh'"
            }
