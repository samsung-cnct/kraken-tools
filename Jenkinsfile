        ...
        customContainer('docker') {
            ...
            stage('Test') {
                parallel (
                    ...
                    echo "no-op" //kubesh "PWD=`pwd` && docker run --rm -i  -e JOB_BASE_NAME=${env.JOB_BASE_NAME} -e BUILD_ID=${env.BUILD_ID} k2-tools:${env.JOB_BASE_NAME}.${env.BUILD_ID} /bin/bash -c /gke-testing.sh"
                    }
                )
            }
